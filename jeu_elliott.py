def jeu_ell():
    ...
    player = pyg.transform.scale(player, (60*screen_rect.width/1080, 40*screen_rect.height/1080)).convert_alpha()
    player_norm = player
    # Importation des images de l'avatar selon les angles
    player_up = pyg.image.load("img_elliott/avion2_up.png")
    player_up = pyg.transform.scale(player_up, (60*screen_rect.width/1080, 60*screen_rect.height/1080)).convert_alpha()
    player_down = pyg.image.load("img_elliott/avion2_down.png")
    player_down = pyg.transform.scale(player_down, (60*screen_rect.width/1080, 60*screen_rect.height/1080)).convert_alpha()
    ...
    feu_haut, feu_bas, feu_gauche, feu_droit = pyg.Rect.copy(screen_rect), pyg.Rect.copy(screen_rect), pyg.Rect.copy(screen_rect), pyg.Rect.copy(screen_rect) # On créé les 4 rectangles de la zone rouge
    ...
    while running:
        ...
        for event in pyg.event.get():
            ...
            if event.type == pyg.KEYDOWN:
                ...
                player_rect_norm.topleft, player_rect_tilt.topleft = (x_player+offset1/2, y_player+offset1/2), (x_player+offset2/2, y_player+offset2/2)
                ...
                if point == 50 and niveau_de_danger == 0:
                    ...
                    niveau_de_danger += 1
                    pyg.Rect.inflate_ip(safezone_rect, -shrink_width, -shrink_height)
                    feu_haut.height, feu_bas.topleft, feu_bas.height = feu_haut.height + shrink_height/2, (0, screen_rect.height-shrink_height), feu_haut.height + shrink_height/2
                    feu_gauche.width, feu_droit.topleft, feu_droit.width = feu_gauche.width + shrink_width/2, (screen_rect.width-shrink_width, 0), feu_gauche.width + shrink_width/2
